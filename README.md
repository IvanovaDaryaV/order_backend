...
- **Controllers**
  ...
  - StatsController - методы с аналитикой по выполнению задач пользователем
...
- **ML** -> "predict overlload days" - папка с ML на python
...
# Загрузка из фала .ics (modeus, LMS)
...
Задачи наследуют контекст проекта, к которому привязываются, но имеют свой, если не привязаны. 

При передаче значения project_id заметке зануляется её поле tag.
...
На серверной части приложения можно отправлять запросы на обучение модели и получение прогноза завала на 30 дней вперед от выбранной даты. Учитываются параметры:
1. кол-во просроченных задач за 3 дня до даты
2. кол-во задач с высоким приоритетом в эту дату
3. ср вр выполнения задач
4. кол-во активных задач, созданных когда-либо до этой даты
5. все задачи, запланированные на эту дату (hardDeadline)

**Эндпоинты на бэкенде:**
1. /api/fit-model-ML - нужно вызывать первым. Генерятся данные, обучается модель. Модель сохраняется файлом.
2. /api/overload-prediction-ML - получение предсказания используя модель из файла выше. Принимает параметры user_id и targetDateString (выбранная дата в формате строки типа dd.MM.yyyy).
...
Вероятность завала в каждый день.

**КАК ЗАПУСКАТЬ:**

1. В папке проекта есть папка ML -> "predict overlload days". Из неё выполнить команду: "uvicorn main:app --host 0.0.0.0 --port 8000 --reload", чтобы запустить сервер.
2. Когда сервер запущен, можно с бэкенда отправлять запросы на эндпоинты /api/fit-model-ML и /api/overload-prediction-ML. В них отправляются запросы на ML сервис автоматически и ответ передается в эндпоинт.
